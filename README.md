...
If the `/outlook/device` endpoint returns this error, the service could not
reach Microsoft’s servers or the `OUTLOOK_CLIENT_ID` is invalid. Verify network
access and ensure a valid client ID is configured (or rely on the default
public client ID).
